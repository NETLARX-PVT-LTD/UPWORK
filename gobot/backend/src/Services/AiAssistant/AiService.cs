        ...
        public async Task<AiGenerateReponseResult> GenerateTextAsync(GenerateRequest request, Errors errors)
        {
            ...
            try
            {
                ...
                if (!response.IsSuccessStatusCode)
                {
                    ...
                    return new AiGenerateReponseResult(false, response.StatusCode.ToString(), string.Empty, errors);
                }
                ...
                if (doc.RootElement.TryGetProperty("choices", out JsonElement choices) && choices.ValueKind == JsonValueKind.Array)
                {
                    foreach (var choice in choices.EnumerateArray())
                    {
                        if (choice.TryGetProperty("message", out JsonElement message) &&
                            message.TryGetProperty("content", out JsonElement contents))
                        {
                            generatedText = contents.GetString() ?? string.Empty;
                ...
                if (string.IsNullOrEmpty(generatedText))
                {
                    _logger.LogWarning("OpenAI API returned no content in 'choices'");
                    errors.Fill(FailureCode.ExternalApiError, "No content returned from AI.");
                    return new AiGenerateReponseResult(false, "204", string.Empty, errors);
                }
