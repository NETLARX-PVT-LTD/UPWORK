...
router.route('/')
  ...
  .post(authenticate,upload, createDocument);
...
const getNextNumber = async () => {
  try {
    ...
    // Generate next number with padding (4 digits)
...
router.post('/', authenticate, upload, async (req, res) => {
  try {
    ...
    if (!description || !sender || !recipient) {
      return res.status(400).json({ 
        message: "Description, sender, and recipient are required fields" 
      });
    }
    ...
    if (req.files && req.files.length > 0) {
      // For Cloudinary files, use the secure_url or path
      // For local files, use the path
      doc.attachments = req.files.map(file => {
        if (file.path && file.path.includes('cloudinary.com')) {
          // Cloudinary file - store the full URL
          return file.path;
        } else {
          // Local file - store relative path
          return file.path;
        }
      });
    }
    ...
    // Add audit log for document registration
    await loggingService.logDocument(
      user, 
...
router.delete('/reserved-numbers/:id', authenticate, authorizeRole('admin'), async (req, res) => {
  try {
    ...
    // Store reservation info before deletion for logging
    const reservationNumber = reservation.number;
    
    await reservation.deleteOne();
    
    // Add audit log for reservation deletion
    await loggingService.logDocument(
      req.user,
      `Deleted reserved number ${reservationNumber}`
    );
...
// Get department documents - All documents from user's department
router.get('/department-documents', authenticate, async (req, res) => {
  try {
    ...
    // Filter documents from the same department
    const departmentDocuments = documents.filter(doc => 
      doc.user && doc.user.department === userDepartment
    );
...
// Get all documents for viewing (read-only, all users)
router.get('/all-readonly', authenticate, async (req, res) => {
  try {
    const documents = await Document.find()
      .populate('user', 'name email department')
      .sort({ createdAt: -1 });
    ...
    console.error('Error fetching all documents for readonly view:', error);
    res.status(500).json({ message: 'Failed to fetch documents' });
  }
});
...
router.put('/:id', authenticate, authorizeRole('admin'), async (req, res) => {
  try {
    ...
    // Add audit log for document update
    await loggingService.logDocument(
      req.user,
...
router.delete('/:id', authenticate, authorizeRole('admin'), async (req, res) => {
  try {
    ...
    // Add audit log for document deletion
    await loggingService.logDocument(
      req.user,
...
// Serve uploaded files for viewing (without forcing download)
router.get('/view/:filename', authenticate, (req, res) => {
  try {
    const filename = decodeURIComponent(req.params.filename);
    
    // Check if it's a Cloudinary URL (contains cloudinary.com) - for images
    if (filename.includes('cloudinary.com')) {
      // For Cloudinary images, redirect to the secure URL
      return res.redirect(filename);
    }
    
    // Handle local files (PDFs and fallback images)
    let filePath;
    
    // Check if filename already includes uploads/ (from stored path)
    ...
    // Don't set Content-Disposition for viewing
...
// Serve uploaded files with authentication for download
router.get('/files/:filename', authenticate, (req, res) => {
  try {
    ...
    // Check if it's a Cloudinary URL (contains cloudinary.com) - for images
    if (filename.includes('cloudinary.com')) {
      // For Cloudinary images, add download parameter to force download
      const downloadUrl = filename.includes('?') 
        ? `${filename}&fl_attachment=document` 
        : `${filename}?fl_attachment=document`;
    ...
    // Extract actual filename for download
