...
$('#pdf-to-jpg-form').on('submit', function(e) {
    ...
    formData.append('nonce', dw_ajax.pdf_to_jpg_nonce);

    $('#pdf-to-jpg-status').text('Processing... Please wait, this may take a moment.').css('color', 'blue');
    $('#download-jpgs-container').empty().hide(); // Clear and hide previous links
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                var $container = $('#download-jpgs-container');
                $container.empty(); // Clear any previous links

                if (response.data.split_files && response.data.split_files.length > 0) {
                    // Assuming 'split_files' is used for a list of output files
                    $.each(response.data.split_files, function(index, file) {
                        $container.append('<p><a href="' + file.download_url + '" download style="background-color: #28a745; color: white; padding: 8px 12px; text-decoration: none; border-radius: 4px; display: inline-block; margin: 5px;">Download Page ' + file.page_number + ' as JPG</a></p>');
                    });
                    $container.show();
                } else {
                    $('#pdf-to-jpg-status').text('Conversion completed, but no JPG files to download.').css('color', 'orange');
                }
            } else {
                $('#pdf-to-jpg-status').text('Error: ' + response.data.message).css('color', 'red');
                $('#download-jpgs-container').hide();
            }
        },
        error: function(jqXHR, textStatus, errorThrown) {
...
// --- PDF to PDF/A Form Handler ---
$('#pdf-to-pdfa-form').on('submit', function(e) {
    e.preventDefault();

    var formData = new FormData(this);
    formData.append('action', 'dw_pdf_to_pdfa');
    formData.append('nonce', dw_ajax.pdf_to_pdfa_nonce); // Make sure to add this nonce in wp_localize_script

    $('#pdf-to-pdfa-status').text('Processing... Please wait, this may take a moment.').css('color', 'blue');
    $('#download-pdf-to-pdfa-container').hide();
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                $('#pdf-to-pdfa-status').text(response.data.message).css('color', 'green');
                $('#download-pdf-to-pdfa').attr('href', response.data.download_url);
                $('#download-pdf-to-pdfa-container').show();
            } else {
                $('#pdf-to-pdfa-status').text('Error: ' + response.data.message).css('color', 'red');
                $('#download-pdf-to-pdfa-container').hide();
            }
        },
        error: function(jqXHR, textStatus, errorThrown) {
            $('#pdf-to-pdfa-status').text('An AJAX error occurred: ' + textStatus).css('color', 'red');
            console.error("AJAX Error:", textStatus, errorThrown, jqXHR.responseText);
        }
    });
});
